...
## Model
mod1_WeakPrior <- brm(formula = log(sla) ~ 
                        CHELSA_bio10_1 + 
                        CHELSA_bio10_12 + 
                        (1 | Species), 
                      prior = prior_m1, 
...
## Model
mod1b_WeakPrior <- brm(formula = log(sla) ~ 
                         CHELSA_bio10_1 + 
                         CHELSA_bio10_12 + 
                         (1 | gr(Species, cov = pana_vcv)),
                      prior = prior_m1, 
                      family = gaussian(), 
                      data = pana_trt, 
...
species <- ranef(mod2_WeakPrior, 
                 probs = c(0.025, 0.05, 0.11, 0.89, 0.95, 0.975))$Species
